    ...
    return (
        <Card sx={{ display: 'flex', justifyContent: 'space-between', flexDirection: 'row' }} variant="outlined">
            <CardContent sx={{ display: 'flex', alignItems: 'center' }}>
                {
                    data && data?.imgs && data?.imgs[0] && <Avatar src={data?.imgs[0].url} />
                }
                <Typography component={RouterLink} sx={{ color: 'CaptionText' }} to={`/product/${id}`}>{data && data.name}</Typography>
                <Typography>Price: 2000-,</Typography>
            </CardContent>
            <CardActions>
                <BasketCountButton count={count} simple onChange={basketHandler}/>
